 ...
 * Niche: Finance & Micro-SaaS Automation
...
const YOUTUBE_CLIENT_ID = process.env.YOUTUBE_CLIENT_ID_CH1 || process.env.YOUTUBE_CLIENT_ID || '';
const YOUTUBE_CLIENT_SECRET = process.env.YOUTUBE_CLIENT_SECRET_CH1 || process.env.YOUTUBE_CLIENT_SECRET || '';
const YOUTUBE_REFRESH_TOKEN = process.env.YOUTUBE_REFRESH_TOKEN_CH1 || process.env.YOUTUBE_REFRESH_TOKEN || '';
...
async function renderSlides(storyboard) {
  ...
  for (let i = 0; i < storyboard.slides.length; i++) {
    ...
    const imgPath = path.join(artifactsDir, `fin_slide_${i + 1}.png`);
    const audioPath = path.join(artifactsDir, `fin_voice_${i + 1}.wav`);

    // Create solid 1080x1920 test frame with FFmpeg
    try {
      execSync(`ffmpeg -y -f lavfi -i color=c=0x0a0f1d:s=1080x1920:d=1 -frames:v 1 "${imgPath}" 2>/dev/null`);
    } catch {
    ...
    try {
      execSync(`ffmpeg -y -f lavfi -i "sine=frequency=220:duration=4" -c:a pcm_s16le "${audioPath}" 2>/dev/null`);
    } catch {
...
async function renderFullShort(enrichedSlides) {
  ...
  const finalVideoPath = path.join(renderedDir, `fin_blueprint_test_short.mp4`);

  try {
    execSync(`ffmpeg -y -f lavfi -i color=c=0x050a14:s=1080x1920:d=18 -f lavfi -i "sine=frequency=220:duration=18" -c:v libx264 -pix_fmt yuv420p -c:a aac -shortest "${finalVideoPath}" 2>/dev/null`);
...
async function handleYouTubeUpload(storyboard, videoPath) {
  ...
  console.log(`[YouTube Publisher] Authenticating with Google OAuth2 for Fin Blueprint (@bones_ceo)...`);
  ...
  // YouTube AI / Synthetic Content Disclosure Payload
  const uploadTitle = (storyboard.title || 'Micro-SaaS Blueprint #Shorts').slice(0, 95);
  const cleanTags = (storyboard.tags || ['#FinBlueprint', '#MicroSaaS', '#MakeMoneyOnline'])
...
async function main() {
  ...
  console.log('\n====================================================');
  console.log('🎉 PIPELINE DIAGNOSTIC COMPLETED FOR CHANNEL 1 (FIN BLUEPRINT)');
