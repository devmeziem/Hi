...
/**
 * Procedural Fallback has been REMOVED per user directive.
 * All scripts must be generated dynamically by active AI inference models.
 */
function generateProceduralFallback(channelId, customTopic) {
  const errorMsg = `[Dynamic Synthesizer FATAL] Procedural / preset fallback scripts are strictly removed. Channel: ${channelId}, Topic: ${customTopic || 'Auto-Topic'}. Please ensure at least one AI provider key is configured.`;
  console.error(`\n${errorMsg}\n`);
